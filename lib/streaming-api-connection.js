...
StreamingAPIConnection.prototype._startPersistentConnection = function () {
  ...
  self.fetch = fetch.post(this.reqOpts);
  self.emit('connect', self.fetch);
  self.fetch.on('response', function (response) {
    ...
    if (STATUS_CODES_TO_ABORT_ON.indexOf(self.response.statusCode) !== -1) {
      ...
      self.fetch.on('data', function (chunk) {
        body += chunk;
      })

      self.fetch.on('end', function () {
        try {
      ...
      self.fetch.on('error', function (err) {
      ...
      self.fetch.on('data', function(data) {
  ...
  self.fetch.on('close', self._onClose.bind(self));
  self.fetch.on('error', function (err) { self._scheduleReconnect.bind(self) });
  return self;
}
...
StreamingAPIConnection.prototype._scheduleReconnect = function () {
  ...
  self.emit('reconnect', self.fetch, self.response, self._connectInterval);
}
