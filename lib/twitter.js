...
Twitter.prototype.get = function (path, params, callback) {
  return this.fetch('GET', path, params, callback)
}

Twitter.prototype.post = function (path, params, callback) {
  return this.fetch('POST', path, params, callback)
}

Twitter.prototype.delete = function (path, params, callback) {
  return this.fetch('DELETE', path, params, callback)
}

Twitter.prototype.fetch = function (method, path, params, callback) {
  var self = this;
...
/**
 * Builds and returns an options object ready to pass to `fetch()`
 ...
 * @returns {Undefined}
 *
 * Calls `callback` with Error, Object where Object is an options object ready to pass to `fetch()`.
 *
...
Twitter.prototype._buildReqOpts = function (method, path, params, isStreaming, callback) {
  ...
  // the options object passed to `node-fetch` used to perform the HTTP request
  var reqOpts = {
    headers: {
      'Accept': '*/*',
 ...
 * @param  {Object}   reqOpts     options object passed to `fetch()`
...
Twitter.prototype._doRestApiRequest = function (reqOpts, twitOptions, method, callback) {
  var fetch_method = fetch[method.toLowerCase()];
  var req = fetch_method(reqOpts);
  ...
  req.on('response', function (res) {
    response = res
    // read data from `node-fetch` object which contains the decompressed HTTP response body,
  ...
  req.on('error', function (err) {
    ...
    if (twitOptions.retry &&
      ...
      self.fetch(method, path, params, callback);
      return;
    } else {
